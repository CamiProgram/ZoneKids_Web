...
public class FullrestApplication { 
     ...
     */
    @Bean
    public CommandLineRunner initData(UserRepository userRepository, ProductoRepository productoRepository, EntityManager entityManager) {
        return args -> {
            try {
                // Limpiar datos anteriores usando SQL nativo para evitar problemas con enums
                System.out.println("🔄 Limpiando datos anteriores...");
                entityManager.createNativeQuery("DELETE FROM productos").executeUpdate();
