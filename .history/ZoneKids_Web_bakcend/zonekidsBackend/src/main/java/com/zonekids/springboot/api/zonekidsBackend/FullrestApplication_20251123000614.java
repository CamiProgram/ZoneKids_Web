...
public class FullrestApplication { 
    ...
    public CommandLineRunner initData(UserRepository userRepository, ProductoRepository productoRepository) {
        return args -> {
            ...
            adminUser.setContrasena("admin123456"); // Se encriptará en el service
            ...
            // URL de imagen por defecto (puedes cambiar a URLs reales)
            ...
            for (int i = 0; i < 10; i++) {
                ...
                producto.setStock(Math.max(5, 20 - (i * 2))); // Stock variado
                producto.setCategoria(categorias[i]);
                producto.setEstado("activo");
                producto.setEsNuevo(i < 3); // Los primeros 3 son "nuevos"
                producto.setEnOferta(i % 2 == 0); // Alternados en oferta
                producto.setPrecioOriginal(precios[i] * 1.2); // 20% de descuento
