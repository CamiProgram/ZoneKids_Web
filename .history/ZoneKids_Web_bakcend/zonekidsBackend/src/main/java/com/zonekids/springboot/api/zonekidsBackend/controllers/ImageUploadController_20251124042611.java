...
public class ImageUploadController {
    ...
    public ResponseEntity<?> uploadImages(@RequestParam(value = "files", required = false) MultipartFile[] files) {
        try {
            if (files == null || files.length == 0) {
                Map<String, Object> error = new HashMap<>();
                error.put("success", false);
                error.put("error", "No se proporcionaron archivos. Asegúrate de enviar el parámetro 'files' en multipart/form-data");
                error.put("status", HttpStatus.BAD_REQUEST.value());
                return ResponseEntity.badRequest().body(error);
            }
    ...
    /**
     * Endpoint alternativo para subir múltiples imágenes (compatibilidad)
     * Acepta parámetro 'imagenesArray' como alternativa a 'files'
     */
    @PostMapping("/subir-imagenes")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Subir múltiples imágenes (alternativo)", description = "Sube múltiples imágenes al servidor (Solo ADMIN) - Parámetro: imagenesArray")
    public ResponseEntity<?> uploadImagesAlternative(@RequestParam(value = "imagenesArray", required = false) MultipartFile[] imagenesArray,
                                                      @RequestParam(value = "files", required = false) MultipartFile[] files) {
        // Usar el parámetro que se proporcionó
        MultipartFile[] filesToProcess = (files != null && files.length > 0) ? files : imagenesArray;
        
        try {
            if (filesToProcess == null || filesToProcess.length == 0) {
                Map<String, Object> error = new HashMap<>();
                error.put("success", false);
                error.put("error", "No se proporcionaron archivos. Usa 'files' o 'imagenesArray' en multipart/form-data");
                error.put("status", HttpStatus.BAD_REQUEST.value());
                return ResponseEntity.badRequest().body(error);
            }
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("totalFiles", filesToProcess.length);
            ...
            for (MultipartFile file : filesToProcess) {
                try {
                    if (!file.isEmpty() && isImageFile(file.getContentType())) {
                        ...
                        Map<String, Object> errorInfo = new HashMap<>();
                        errorInfo.put("fileName", file.getOriginalFilename());
                        errorInfo.put("error", "Archivo vacío");
                        errors.add(errorInfo);
                    } else {
                        Map<String, Object> errorInfo = new HashMap<>();
                        errorInfo.put("fileName", file.getOriginalFilename());
                        errorInfo.put("error", "Tipo de archivo no permitido: " + file.getContentType());
                        errors.add(errorInfo);
                    }
                } catch (Exception e) {
                    Map<String, Object> errorInfo = new HashMap<>();
