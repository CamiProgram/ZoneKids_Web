...
describe('RegisterPage', () => {
  ...
  it('debe mostrar error si el RUT no tiene 9 dígitos', async () => {
    ...
    await user.type(rutInput, '12345');
    await user.type(emailInput, 'test@example.com');
    await user.type(passwordInput, 'password123');
    await user.click(submitButton);

    // El campo RUT debe tener error
    await waitFor(() => {
      expect(rutInput).toHaveClass('input-error');
    }, { timeout: 2000 });
  });
  ...
  it('debe permitir escribir en todos los campos incluyendo RUT válido', async () => {
    const user = userEvent.setup();
    renderWithAuth(<RegisterPage />);
    ...
    expect(rutInput).toHaveValue('123456789');
