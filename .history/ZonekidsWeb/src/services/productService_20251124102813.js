...
export const productService = {
  ...
  uploadImages: async (files) => {
    ...
    try {
      ...
      // ⭐ IMPORTANTE: Extraer SOLO las URLs, no los objetos completos
      if (response.data.success && response.data.uploadedFiles) {
        // Los archivos vienen como objetos con propiedad .url
        const imageUrls = response.data.uploadedFiles.map(file => {
          if (typeof file === 'string') {
            return file;
          } else if (file.url) {
            return file.url;
          } else {
            console.warn('⚠️ Objeto de archivo sin propiedad .url:', file);
            return file;
          }
        });
        
        console.log('✅ URLs extraídas:', imageUrls.length);
        imageUrls.forEach((url, idx) => {
          console.log(`  🔗 ${idx + 1}. ${url}`);
        });
        
        return imageUrls; // Retorna ['url1', 'url2', 'url3']
      ...
      console.error('❌ uploadImages error:', error.response?.data || error);
      throw error;
    }
  },
