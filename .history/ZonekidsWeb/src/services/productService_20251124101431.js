...
export const productService = {
  ...
  uploadImages: async (files) => {
    try {
      ...
      // Convertir a strings en caso de que sean objetos
      const urlStrings = imagenesUrl.map(url => {
        if (typeof url === 'string') {
          return url;
        } else if (typeof url === 'object' && url !== null) {
          // Si es objeto, intentar extraer la URL
          return url.url || url.path || url.filename || JSON.stringify(url);
        } else {
          return String(url);
        }
      });
      
      console.log('✅ URLs procesadas:', urlStrings.length);
      urlStrings.forEach((url, idx) => {
        const preview = typeof url === 'string' ? url.substring(0, 60) : String(url).substring(0, 60);
