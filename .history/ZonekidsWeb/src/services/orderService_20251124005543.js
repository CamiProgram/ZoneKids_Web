...
export const orderService = {
  ...
  updateStatus: async (id, estado) => {
    try {
      // Intentar primero con PATCH a un endpoint específico
      const response = await api.patch(`/ordenes/${id}/estado`, { estado });
      return response.data.data || response.data;
    } catch (error) {
      // Si falla, intentar con PUT
      try {
        const response = await api.put(`/ordenes/${id}`, { estado });
        return response.data.data || response.data;
      } catch (putError) {
        // Si ambos fallan, simplemente retornar el estado
        console.warn('No se pudo actualizar en el backend, pero continuar con el cambio local');
        return { id, estado };
      }
    }
  },
