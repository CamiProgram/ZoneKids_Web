...
export const productService = {
  ...
  uploadImages: async (files) => {
    ...
    try {
      ...
      // Extraer el array de URLs de la respuesta
      let urlsArray = [];
      
      if (response.data.uploadedFiles && Array.isArray(response.data.uploadedFiles)) {
        // Si viene en uploadedFiles
        urlsArray = response.data.uploadedFiles;
        console.log('✅ URLs extraídas de uploadedFiles:', urlsArray.length);
      } else if (Array.isArray(response.data)) {
        // Si es un array directo
        urlsArray = response.data;
        console.log('✅ URLs extraídas como array directo:', urlsArray.length);
      } else if (response.data.data && Array.isArray(response.data.data)) {
        // Si viene en data
        urlsArray = response.data.data;
        console.log('✅ URLs extraídas de data:', urlsArray.length);
      } else {
        console.error('❌ Estructura de respuesta no reconocida:', response.data);
        throw new Error('La respuesta del servidor no tiene una estructura válida');
      }
      
      if (urlsArray.length === 0) {
        throw new Error('El servidor no retornó URLs de imágenes');
      }
      
      console.log('✅ Retornando', urlsArray.length, 'URLs de imágenes');
      urlsArray.forEach((url, idx) => {
        console.log(`  🔗 ${idx + 1}. ${typeof url === 'string' ? url.substring(0, 50) : JSON.stringify(url)}`);
      });
