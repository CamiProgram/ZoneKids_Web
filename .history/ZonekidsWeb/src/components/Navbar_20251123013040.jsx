...
export const Navbar = () => {
  ...
  // Función para cambiar rol (desarrollo)
  const makeAdmin = () => {
    if (user) {
      const updatedUser = { ...user, rol: 'ADMIN' };
  ...
  return (
    <nav className="navbar-container">
      <div className="navbar-top">
        <NavLink to="/" className="logo" onDoubleClick={() => {
          // Doble click en el logo para abrir dev menu si estás loggeado
      ...
      {/* Dev Menu */}
      {showDevMenu && user && user.email === 'camilotapia828@gmail.com' && (
        <div className="dev-menu">
          <p>🛠️ Modo Desarrollo</p>
          <button onClick={makeAdmin} className="dev-button">
            {user.rol === 'ADMIN' ? '✓ Ya eres ADMIN' : 'Hacer ADMIN'}
          </button>
          <button onClick={() => setShowDevMenu(false)} className="dev-button close">
            Cerrar
          </button>
        </div>
      )}
