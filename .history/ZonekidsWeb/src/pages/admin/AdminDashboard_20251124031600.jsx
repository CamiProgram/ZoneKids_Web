...
────────────────────────────────────────
    ...
    if (window.html2pdf) {
      ...
      // Método alternativo: crear PDF manualmente
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      const width = 210; // A4 width in mm
      const height = 297; // A4 height in mm
      
      // Crear un blob con contenido para descargar como PDF simple
...
endobj
4 0 obj
<< /Length ${receiptContent.length + 100} >>
stream
BT
/F1 12 Tf
50 750 Td
(${receiptContent.replace(/\(/g, '\\(').replace(/\)/g, '\\)')}) Tj
ET
endstream
endobj
5 0 obj
...
0000000115 00000 n
0000000229 00000 n
0000000${receiptContent.length + 329} 00000 n
trailer
<< /Size 6 /Root 1 0 R >>
startxref
${receiptContent.length + 428}
%%EOF`;
      ...
      link.href = url;
      link.setAttribute('download', `boleta_${order.id}.pdf`);
