...
export const CheckoutPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    decrementStockForPurchase(order);
  ...
  const decrementStockForPurchase = async (order) => {
    try {
      for (const item of order.items) {
        const product = cartItems.find(p => p.id === item.id);
        if (product) {
          const newStock = (product.cantidad || product.stock || 0) - item.quantity;
      ...
      // No detener la compra si hay error en actualizar stock
    }
  };
