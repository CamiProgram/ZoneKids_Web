...
export const CrearProducto = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!imagen) { setError('Debes seleccionar una imagen.'); setLoading(false); return; }
        const formData = new FormData();
        const productData = { 
            nombre, descripcion, precio: parseFloat(precio), stock: parseInt(stock), 
            categoria, estado, precioOriginal: precioOriginal ? parseFloat(precioOriginal) : null,
            esNuevo: esNuevo, enOferta: enOferta
        };
        formData.append('product', new Blob([JSON.stringify(productData)], { type: 'application/json' }));
        formData.append('file', imagen);
        try {
            await axios.post('http://localhost:8080/api/products', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            navigate('/admin/products');
        } catch (err) { setError('Error al guardar el producto.'); } 
    ...
    return (
        <div className="admin-form-container">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-group"><label htmlFor="imagen">Imagen del Producto</label><input type="file" id="imagen" onChange={handleImagenChange} accept="image/*" required /></div>
                <div className="form-group-inline">
                ...
                <button type="submit" disabled={loading} className="btn-submit">{loading ? 'Guardando...' : 'Crear Producto'}</button>
            </form>
        </div>
    );
};